...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const sumVec = x1 * x2 + y1 * y2;
  const sumFirst = Math.sqrt(x1 * x1 + y1 * y1);
  const sumSecond = Math.sqrt(x2 * x2 + y2 * y2);
  const res = sumFirst * sumSecond;
  return Math.acos(sumVec / res);
}
...
function getSumOfDigits(num) {
  ...
  return res.reduce((s, cur) => s + parseInt(cur, 10), 0);
}
